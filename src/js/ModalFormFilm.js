...
function addArrowEventlisteners() {
  document.querySelectorAll('.lightbox__arrow').forEach(item => {
    item.addEventListener('click', event => {
      makeLightbox(galleryList[item.dataset.next][0], galleryList[item.dataset.next][1]);
    });
  });
}

function addCloseBtnEventlistener() {
  document.querySelector('.lightbox__close-btn').addEventListener('click', event => {
...
function addTabs() {
  new ItcTabs(document.querySelector('.tabs'));
